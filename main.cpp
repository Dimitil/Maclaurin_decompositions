...
double _pow(double a, double x, int precision = defPrecision)
{
	...
	double ln_a = log(a); //çäåñü íå çíàþ ÷òî ïðèäóìàòü (êðîìå âûçîâà íàïèñàííîé ôóíêöèè ne znau chto pridumat' crome vizova funcii nije _log(a))
	for (int i = 1; i <= precision; i++)
	{
		sum += member;
...
double logFrom1MinusX(double x, int precision = defPrecision) //ïðîâåðêó íà ââîä ñêàçàëè íå äåëàòü vi skazali, chto proverku na vvod skazali ne delat'
{
...
double _revSqrt(double x, int m=2, int precision = defPrecision) //âîçìîæíî, m äîëæíà áûòü òîëüêî ÷åòíîé vozmojno m doljna bbit' tol'ko chetnoi
{
	double member = 2;
	for (int i = 1; i < m / 2; i++)
	{
